...
function createIssue (title, workItemType, state, description, assignee) {
  var body = {
    title: title,
    body: description,
    assignees: [getGitHubUser(assignee)],
    labels: [workItemType, state]
  }
...
try {
  fs.readFile('wit.csv', function (err, charBuffer) {
    ...
    for (var i = 2; i < lines.length; i++) {
      var id, workItemType, title, user, state
      ...
      if (id > 0) {
        createIssue(title, workItemType, state, description, user)
      }
    }
  })
} catch (err) {
